...
class InstallModule(threading.Thread):
    ...
    def run(self):
        try:
            ...
            _LOGGER.error(f"安装「{self.module_name}」模块失败, 原因为：{e}", exc_info=True)
        _LOGGER.info(f"安装「{self.module_name}」模块完成！")
...
def GetPackage(module_name):
    ...
    try:
        ...
        _LOGGER.warning(f"正在安装「{module_name}」模块，如日志无报错，稍后自行重启容器生效。")
    except Exception as e:
        _LOGGER.error(f"安装「{module_name}」模块失败, 原因为：{e}", exc_info=True)
